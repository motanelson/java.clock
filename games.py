...
class GameEngine:
    ...
    def redraws(self):
        ...
        for i in range(len(frames) - 1):
            ...
            # paredes esquerda e direita (linhas verticais inclinadas)
            # esquerda: entre (x1,y1)->(x1,y2) e (xn1,yn1)->(xn1,yn2)
            self._draw_running_bond_on_wall((x1, y1), (x1, y2), (xn1, yn1), (xn1, yn2), t)
            # direita: entre (x2,y1)->(x2,y2) e (xn2,yn1)->(xn2,yn2)
            self._draw_running_bond_on_wall((x2, y1), (x2, y2), (xn2, yn1), (xn2, yn2), (t + 0.5) % 1.0)

            # teto e chão: várias subdivisões para simular tijolos
            # definimos quantas juntas verticais nesta camada (mais ao fundo -> mais juntas)
            cols = max(1, int(2 + i * 0.15))
            for k in range(1, cols + 1):
                u = ((k / (cols + 1)) + t) % 1.0
                # topo: entre (x1,y1)->(x2,y1) e (xn1,yn1)->(xn2,yn1)
                a_top = (x1 + (x2 - x1) * u, y1)
                b_top = (xn1 + (xn2 - xn1) * u, yn1)
                c.create_line(a_top[0], a_top[1], b_top[0], b_top[1], width=max(1, self.line_width-1), fill="black")
                # chão: entre (x1,y2)->(x2,y2) e (xn1,yn2)->(xn2,yn2)
                a_bot = (x1 + (x2 - x1) * u, y2)
                b_bot = (xn1 + (xn2 - xn1) * u, yn2)
                c.create_line(a_bot[0], a_bot[1], b_bot[0], b_bot[1], width=max(1, self.line_width-1), fill="black")

            # a cada camada, deslocamos ligeiramente a fase para dar efeito de 'desencontro' dos tijolos
            t = (t + 0.18) % 1.0
